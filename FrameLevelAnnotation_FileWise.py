...
mfcc_vector_output_filedir=sys.argv[3]
class_label_vector_output_filedir=sys.argv[4]

speech_vector_class_label_list = []
	...
	for root, dirs, files in os.walk(annotation_dir):
		...
		for file in files:
			...
			if (file.lower().endswith('.wav')):
				...
				class_label_vector_filename = str('class_label_'+os.path.splitext(file)[0])+'.dat'

				mfcc_vector_output_filedir_final = os.path.join(mfcc_vector_output_filedir, root_dir_name)
				class_label_vector_output_filedir_final =  os.path.join(class_label_vector_output_filedir, root_dir_name)

				if not os.path.exists(mfcc_vector_output_filedir_final):
					os.makedirs(mfcc_vector_output_filedir_final)
				if not os.path.exists(class_label_vector_output_filedir_final):
					os.makedirs(class_label_vector_output_filedir_final)

				mfcc_vector_output_file = os.path.join(mfcc_vector_output_filedir_final,mfcc_vector_filename)
				class_label_vector_output_file = os.path.join(class_label_vector_output_filedir_final,class_label_vector_filename)
